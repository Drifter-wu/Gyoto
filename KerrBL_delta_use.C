...
void KerrBL::gmunu(double g[4][4], const double * pos) const {
	...
	double sigma_delta = pow((pow(m, 2) - pow(as, 2)), 0.5);
	double x = (r - m) / sigma_delta;
	double y = pow(cth2, 1 / 2);
	double q = delta - 1;
	double alpha = (m - sigma_delta) / as;
	double lambda = alpha * pow((pow(x, 2) - 1), -q)*pow((x + y), (2 * q));
	double eta = alpha * pow((pow(x, 2) - 1), -q)*pow((x - y), (2 * q));
	double aplus = pow((x + 1), q)*(x*(1 - lambda * eta) + (1 + lambda * eta));
	double amins = pow(-1 + x, q)*(-1 - eta * lambda + (1 - eta * lambda)*x);
	double bplus = pow(1 + x, q)*(eta - lambda + (eta + lambda)*y);
	double bmins = pow((x - 1), q)*((eta + lambda)*y + (-eta + lambda));
	double PlA = aplus * amins + bplus * bmins;
	double PlB = pow(aplus, 2) + pow(bplus, 2);
	double PlC = pow(1 + x, q)*(bplus*(1 - eta * lambda)*(-1 + pow(x, 2))*y +
		aplus * (eta + lambda)*x*(1 - pow(y, 2)));
	double F = PlA / PlB;
	double omega_delta = 2 * (as - (PlC*sigma_delta) / PlA);
	double e2gamma = (PlA*pow(1 + m / sigma_delta, 2)*pow((-1 + pow(x, 2)) / (pow(x, 2) - pow(y, 2)), pow(delta, 2))) /
		(4.*pow(-1 + pow(x, 2), delta));
	double mathA = 1 + pow(as, 2) / pow(r, 2) - (2 * m) / r;
	double mathB = mathA + pow(sigma_delta, 2)*sth2 / (r*r);

	g[0][0] = -F;
	g[1][1] = (e2gamma / F)*mathB / mathA;
	g[2][2] = r * r*(e2gamma*mathB) / F;
	g[3][3] = r * r*mathA*sth2 / F - F * pow(omega_delta, 2);
	g[0][3] = g[3][0] = F * omega_delta;
}
...
double KerrBL::gmunu(const double * pos, int mu, int nu) const {
	...
	if ((mu == 0) && (nu == 0)) return -F;
	if ((mu == 1) && (nu == 1)) return (e2gamma / F)*mathB / mathA;
	if ((mu == 2) && (nu == 2)) return r * r*(e2gamma*mathB) / F;
	if ((mu == 3) && (nu == 3))
		return r * r*mathA*sth2 / F - F * pow(omega_delta, 2);
	if (((mu == 0) && (nu == 3)) || ((mu == 3) && (nu == 0))) {
		return F * omega_delta;
	}
...
void KerrBL::gmunu_up(double gup[4][4], const double * pos) const {
	...
	gup[0][0] = -((pow(F, 2)*(1 / sth2)*(-((pow(e2gamma, 2)*pow(mathB, 2)*pow(omega_delta, 2)*pow(r, 2)) /
		(F*mathA)) + (pow(e2gamma, 2)*pow(mathB, 2)*pow(r, 4)*sth2) /
		pow(F, 3))) / (pow(e2gamma, 2)*pow(mathB, 2)*pow(r, 4)));
	gup[1][1] = (F*mathA) / (e2gamma*mathB);
	gup[2][2] = F / (e2gamma*mathB*pow(r, 2));
	gup[3][3] = (F*(1 / sth2)) / (mathA*pow(r, 2));
	gup[0][3] = gup[3][0] = (F*omega_delta*(1 / sth2)) / (mathA*pow(r, 2));
...
double KerrBL::gmunu_up(const double * pos, int mu, int nu) const {
	...
	if ((mu == 0) && (nu == 0)) {
		return -((pow(F, 2)*(1 / sth2)*(-((pow(e2gamma, 2)*pow(mathB, 2)*pow(omega_delta, 2)*pow(r, 2)) /
			(F*mathA)) + (pow(e2gamma, 2)*pow(mathB, 2)*pow(r, 4)*sth2) /
			pow(F, 3))) / (pow(e2gamma, 2)*pow(mathB, 2)*pow(r, 4)));;
	}
	if ((mu == 1) && (nu == 1)) return (F*mathA) / (e2gamma*mathB);
	if ((mu == 2) && (nu == 2)) return F / (e2gamma*mathB*pow(r, 2));
	if ((mu == 3) && (nu == 3)) {
		return (F*(1 / sth2)) / (mathA*pow(r, 2));
	}
	if (((mu == 0) && (nu == 3)) || ((mu == 3) && (nu == 0))) {
		return (F*omega_delta*(1 / sth2)) / (mathA*pow(r, 2));
	}
